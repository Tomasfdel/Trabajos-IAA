...
int arquitec(char *filename,struct MLP *nn,struct DATOS *datos){
  ...
  fscanf(b,"%d",&nn->BATCH);
  nn->BATCH = min(nn->BATCH, datos->PR);
...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  //~ Variable auxiliar para calcular los gradientes de la capa 2 durante el entrenamiento.
  float grad3Aux[nn->N3 + 1];
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
	  ...
	  //~ Se reinician los valores de los gradientes de cada capa, que se usarán como acumuladores para el batch.
	  for(i=1; i<=nn->N2; i++)
		nn->grad2[i] = 0;
	  ...
	  //~ Se itera mientras no se haya completado un batch o se quede sin datos de entrenamiento en la época actual.
	  for(nu2 = 0; nu1 < datos->PR && nu2 < nn->BATCH; nu1++, nu2++){
		  ...
		  for( i=1; i<=nn->N3; i++){
			 grad3Aux[i] = (nn->target[i] - nn->x3[i]);  /*corresponde a lineal*/
			 nn->grad3[i] += grad3Aux[i];
		  }
