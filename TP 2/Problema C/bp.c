...
struct MLP {
    ...
    float u;            /* Momentum */
    float GAMMA;        /* Parámetro de Weight Decay */
...
int train(struct MLP *nn,struct DATOS *datos){
  ...
  // Valor de penalización al valor de los pesos, usado en la actualización de los mismos.
  float weightDecay = 1 - 2 * nn->GAMMA * eta;
  ...
  for (iter=1; iter<=nn->ITER; iter++) {
    ...
    for(nu1=0;nu1<datos->PR;nu1++) {
	  ...
	  for( i=1; i<=nn->N3; i++) for( j=0; j<=nn->N2; j++){
		...
		nn->w2[i][j] = weightDecay * nn->w2[i][j] + nn->dw2[i][j];                         /*correccion*/
	  }
	  ...
	  for( j=1; j<=nn->N2; j++) for( k=0; k<=nn->N1; k++){
		...
		nn->w1[j][k] = weightDecay * nn->w1[j][k] + nn->dw1[j][k];                         /*correccion*/
	  }
    ...
    if ((iter % nn->NERROR) == 0) {
      ...
      /*calcular mse de validacion; si no hay validacion, calcular la suma de pesos de la red*/
      ...
      /*conservo los pesos de la red que da la minima suma total de pesos*/ 
