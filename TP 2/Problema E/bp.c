...
int read_data(char *filenamepasado,struct DATOS *datos){
  ...
  for(k=0;k<datos->PTOT;k++){
    ...
    //~ Se lee la clase del dato actual.
    fscanf(fpat,"%d",&clase);
    if(CONTROL>1) printf(" - ");
    //~ Se crea el vector contra el que se comparan las salidas de la red.
    //~ Tiene un 0.9 en la posición de la clase correspondiente y 0.1 en los demás.
    for(j = 0; j < datos->N3; j++){
      if(j == clase)
          valor = 0.9;
      else
          valor = 0.1;
      datos->data[k][datos->N1 + j + 1] = valor;

      if(CONTROL>1) printf("%f\t", valor);
      separador=getc(fpat);
      if(separador!=',') ungetc(separador,fpat);
    }
  }
  ...
  for(k=0;k<datos->PTEST;k++){
    ...
    for(j = 0; j < datos->N3; j++){
      if(j == clase)
          ...
          valor = 0.1;
      datos->test[k][datos->N1 + j + 1] = valor;
  ...
  *discrete_error=*mse=0.0;
  ...
  for (patron=pat_ini; patron<pat_fin; patron++) { 
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    //~ Se busca cuál es el índice del vector que tiene la máxima activación.
    indexMaxActivacion = 1;
    maxActivacion = nn->x3[1];
    for(i = 1; i <= nn->N3; i++) {
        *mse += (nn->x3[i]-S[nu][nn->N1+i])*(nn->x3[i]-S[nu][nn->N1+i]);                           /*mse se calcula para todas las salidas*/
        if(nn->x3[i] > maxActivacion){
			maxActivacion = nn->x3[i];
			indexMaxActivacion = i;
		}
    }
    
    //~ Si el índice de mayor activación no es un 0.9, entonces se cometió un
    //~ error en la clasificación.
    //~ Se usa 0.5 en vez de 0.9 para evitar errores de igualdad entre floats,
    //~ pero la idea del if es revisar si el índice activado no tiene un 0.9.
    if(S[nu][nn->N1 + indexMaxActivacion] < 0.5)
		*discrete_error += 1;
  }
