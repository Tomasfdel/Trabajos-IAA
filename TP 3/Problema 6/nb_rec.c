...
//~ Lista enlazada usada para la implementación recursiva de la búsqueda de thresholds.
...
struct NB {
    ...
    List **featThresholds; // Lista de valores de los thresholds para cada atributo.
...
//~ newList: Devuelve una lista vacía.
List* newList(){
...
//~ listIsEmpty: Determina si una lista está vacía.
int listIsEmpty(List *list){
	return list -> length == 0;
}

//~ listLength: Devuelve el largo de una lista.
int listLength(List *list){
	return list -> length;
}

//~ listAppend: Agrega un elemento al final de una lista.
void listAppend(List *list, float newVal){
	if(listIsEmpty(list)){
		list -> first = list -> last = (Node *) malloc(sizeof(Node));
		list -> first -> data = newVal;
		list -> first -> next = NULL;
	}
	else{
		list -> last -> next = (Node *) malloc(sizeof(Node));
		list -> last = list -> last -> next;
		list -> last -> data = newVal;
		list -> last -> next = NULL;
	}
	list -> length ++;
}

//~ getIntervalIndex: Dado un valor, devuelve el índice del intervalo 
//~ del histograma al que pertenece.
int getIntervalIndex(List *list, float value){		
	Node *current;
	int counter;
	//~ Itera mientras no llegue al final de la lista y el valor sea mayor a los valores de los thresholds.
	for(current = list -> first, counter = 0; current != NULL && value > current -> data; current = current -> next, counter ++);
...
int define_matrix_nb(struct NB *nb){
  /*ALLOCAR ESPACIO PARA LAS MATRICES DEL CLASIFICADOR
   Listados de thresholds, probabilidades por feature
   y probabilidades a priori. */
	int i; 
	
	nb -> featThresholds = (List **) calloc(nb -> N_IN, sizeof(List *));
	nb -> featProb = (float ***) calloc(nb -> N_IN, sizeof(float **));
	nb -> classProb = (float *) calloc(nb -> N_Class, sizeof(float));
	
	if(!nb -> featThresholds || !nb -> featProb || !nb -> classProb) return 1;
	
	//~ No defino todavía el último nivel porque no sé todavía
	//~ cuántos bins se usarán en cada atributo.
	for(i = 0; i < nb -> N_IN; i++){
...
int arquitec(char *filename,struct NB *nb,struct DATOS *datos){
  ...
  printf("\nNaive Bayes con discretizacion recursiva:\nCantidad de entradas:%d",nb->N_IN);
...
float prob(struct NB *nb,float x,int feature,int clase){
	...
	int intervalIndex = getIntervalIndex(nb -> featThresholds[feature], x);
...
//~ attributeCompare: Devuelve el orden entre un atributo específico de dos valores.
//~ attributeToSort tiene el índice del atributo sobre el que se quiere ordenar,
//~ y va siendo modificado en la función de entrenamiento.
int attributeToSort;
int attributeCompare (const void *a, const void *b){
	float *firstVal = *((float **) a);
	float *secondVal = *((float **) b);
	
	if(firstVal[attributeToSort] < secondVal[attributeToSort])
		return -1;
	if(firstVal[attributeToSort] == secondVal[attributeToSort])
		return 0;
...
//~ logBase2: Calcula el logaritmo en base 2 de un valor.
float logBase2(float value){
	return log10(value) / log10(2);
}


//~ intervalEntropy: Calcula la entropía de un intervalo dado.
float intervalEntropy(float **data, struct NB *nb, int first, int last){
	float entropy = 0;
	float proportion;
	int class, i;
	int classCount;
	int intervalSize = last - first + 1;
	
	for(class = 0; class < nb -> N_Class; class ++){
		classCount = 0;
		for(i = first; i <= last; i++)
			if(data[i][nb -> N_IN] == class)
				classCount ++;
			
		if(classCount){
			proportion = (float) classCount / intervalSize;
			entropy += - proportion * logBase2(proportion);  
		}
	}
...
//~ partitionEntropy: Dado un intervalo y un threshold, calcula el E(A, T; S)
//~ como está descripto en el paper.
float partitionEntropy(float **data, struct NB *nb, int first, int threshold, int last){
	int intervalSize = last - first + 1;
	int firstPartSize = threshold - first + 1;
	int secondPartSize = last - threshold;
	float firstEntropy = intervalEntropy(data, nb, first, threshold);
	float secondEntropy = intervalEntropy(data, nb, threshold + 1, last);
	
	return (float)firstPartSize / intervalSize * firstEntropy + (float)secondPartSize / intervalSize * secondEntropy;
}

//~ intervalClassCount: Cuenta la cantidad de clases distintas presentes en el intervalo dado.
int intervalClassCount(float **data, struct NB *nb, int first, int last){
	int *classCounters = (int *) calloc (nb -> N_Class, sizeof(int));
	int i;
	int counter = 0;
	
	for(i = first; i <= last; i++)
		classCounters[(int) data[i][nb -> N_IN]]++;
...
//~ delta: Calcula Delta(A,Tmin;S) según lo descripto en el paper.
float delta(float **data, struct NB *nb, int first, int threshold, int last){
	int totalK = intervalClassCount(data, nb, first, last);
	int firstK = intervalClassCount(data, nb, first, threshold);
	int secondK = intervalClassCount(data, nb, threshold + 1, last);
	
	return logBase2(pow(3, totalK) - 2) - 
	       (totalK * intervalEntropy(data, nb, first, last) - firstK * intervalEntropy(data, nb, first, threshold) - secondK * intervalEntropy(data, nb, threshold + 1, last));
}	

//~ findThresholds: Dados los datos ordenados por un atributo, el atributo correspondiente
//~ y los índices del primer y último elemento, busca recursivamente los puntos de corte 
//~ necesarios para dividir los datos.
void findThresholds(float **data, int attribute, struct NB *nb, int first, int last, List *thresholdList){	
	if(last > first){
		int thresholdIndex;
		int foundThreshold = 0, minThresholdIndex;
		float thresholdValue, entropy;
		float minThresholdValue, minEntropy;
		
		//~ Iteramos en cada par de puntos consecutivos en el intervalo. 
		for(thresholdIndex = first; thresholdIndex < last; thresholdIndex ++){
			//~ Revisamos que sus valores en el atributo a dividir sean distintos, cosa de que se 
			//~ puedan dividir los datos con un punto de corte entre ellos.
			if(data[thresholdIndex][attribute] != data[thresholdIndex + 1][attribute]){
				thresholdValue = (data[thresholdIndex + 1][attribute] + data[thresholdIndex][attribute]) / 2;
				entropy = partitionEntropy(data, nb, first, thresholdIndex, last);
				
				//~ Si no encontré un threshold posible todavía o el threshold actual resulta en una entropía
				//~ menor, lo uso como el Tmin.
				if(!foundThreshold || entropy < minEntropy){
					foundThreshold = 1;
					minThresholdIndex = thresholdIndex;
					minThresholdValue = thresholdValue;
					minEntropy = entropy;
				}
			} 
		}
		
		//~ Si se encontró un threshold posible, consideramos llamar recursivamente.
		if(foundThreshold){
			int intervalLength = last - first + 1;
			//~ Calculo Gain(A,T;S)
			float gain = intervalEntropy(data, nb, first, last) - minEntropy;
			float minValidGain = logBase2(intervalLength - 1) / intervalLength + delta(data, nb, first, minThresholdIndex, last) / intervalLength;
			
			//~ Revisar que el threshold ese me dé una ganancia mayor que el valor de corte	
			//~ Si lo hace, llamar recursivamente en ambos lados.
			if(gain >= minValidGain){
				findThresholds(data, attribute, nb, first, minThresholdIndex, thresholdList);
				listAppend(thresholdList, minThresholdValue);
				findThresholds(data, attribute, nb, minThresholdIndex + 1, last, thresholdList);
			}
		}
	}
} 
...
int train(struct NB *nb,struct DATOS *datos){
  ...
  if(nb->SEED>-1 && N_TOTAL<datos->PTOT){
	...
	//~ Para cada atributo: Ordenar en forma creciente y buscar los thresholds posibles
	//~ para dividirlo.
	for(attribute = 0; attribute < nb -> N_IN; attribute ++){		
		attributeToSort = attribute;
		qsort(datos -> data, N_TOTAL, sizeof(float *), attributeCompare);

		//~ Buscamos los thresholds de ese atributo, los guardamos y creamos 
		//~ contadores para la cantidad de elementos de cada intervalo.
		List *thresholdList = newList();
		findThresholds(datos -> data, attribute, nb, 0, N_TOTAL - 1, thresholdList);
		int intervalAmount = listLength(thresholdList) + 1;
		
		nb -> featThresholds[attribute] = thresholdList;
		
		for(i = 0; i < nb -> N_Class; i++){
			nb -> featProb[attribute][i] = (float *) calloc(intervalAmount, sizeof(float));
			if(!nb -> featProb[attribute][i]) return 1;
		}
	}
	...
	for(i = 0; i < N_TOTAL; i++){
		class = datos -> data[i][datos -> N_IN];
		for(attribute = 0; attribute < nb -> N_IN; attribute ++){
			int intervalIndex = getIntervalIndex(nb -> featThresholds[attribute], datos -> data[i][attribute]);
	...
	for(class = 0; class < nb -> N_Class; class++){
		...
		for(attribute = 0; attribute < nb -> N_IN; attribute ++){	
			int intervalAmount = listLength(nb -> featThresholds[attribute]) + 1;
			for(i = 0; i < intervalAmount; i++){
				nb -> featProb[attribute][class][i] = (nb -> featProb[attribute][class][i] + 1/intervalAmount) / (classCount + 1);
			}
		}
	}
  ...
  printf("Validacion:%f%%\nTest:%f%%\n",valid_error*100.,test_error*100.);
	...
	if(CONTROL){
		...
		for(attribute = 0; attribute < datos -> N_IN; attribute++){
			printf("Atributo %d:", attribute);
			Node* current;
			for(current = nb -> featThresholds[attribute] -> first; current != NULL; current = current -> next)
		...
		for(attribute = 0; attribute < datos -> N_IN; attribute++){
			printf("Atributo %d:\n", attribute);
			int intervalCount = listLength(nb -> featThresholds[attribute]) + 1;
			for(class = 0; class < nb -> N_Class; class++){
