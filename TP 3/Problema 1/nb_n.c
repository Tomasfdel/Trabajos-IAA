...
struct NB {
	...
	float **means, **variances; // Matrices de medias y varianzas.
...
int define_matrix_nb(struct NB *nb){
	...
	nb -> means = (float **) calloc(nb -> N_Class, sizeof(float *));
	nb -> variances = (float **) calloc(nb -> N_Class, sizeof(float *));
	
	nb -> classProb = (float *) calloc(nb -> N_Class, sizeof(float));
	
	if(!nb -> means || !nb -> variances || !nb -> classProb) return 1;
	
	for(i = 0; i < nb -> N_Class; i++){
		nb -> means[i] = (float *) calloc(nb -> N_IN, sizeof(float));
		nb -> variances[i] = (float *) calloc(nb -> N_IN, sizeof(float));
		if(!nb -> means[i] || !nb -> variances[i]) return 1;
	}
...
float prob(struct NB *nb,float x,int feature,int clase){
	...
	float mean = nb -> means[clase][feature];
	float variance = nb -> variances[clase][feature];
	float exponent = - pow(x - mean, 2) / (2 * variance);
	float prob = exp(exponent) / sqrt(2 * PI * variance) ;
...
int train(struct NB *nb,struct DATOS *datos){
  ...
  if(nb->SEED>-1 && N_TOTAL<datos->PTOT){
	...
	//~ Calcular media por cada clase y atributo
	for(i = 0; i < N_TOTAL; i++){
		class = datos -> data[i][datos -> N_IN];
		for(attribute = 0; attribute < datos -> N_IN; attribute++)
			nb -> means[class][attribute] += datos -> data[i][attribute];
	}
	
	for(class = 0; class < nb -> N_Class; class++)
		for(attribute = 0; attribute < datos -> N_IN; attribute++)
			nb -> means[class][attribute] /= nb -> classProb[class];
  
	//~ Calcular varianza por cada clase y atributo
	for(i = 0; i < N_TOTAL; i++){
		class = datos -> data[i][datos -> N_IN];
		for(attribute = 0; attribute < datos -> N_IN; attribute++)
			nb -> variances[class][attribute] += pow(datos -> data[i][attribute] - nb -> means[class][attribute], 2);
	}
	
	for(class = 0; class < nb -> N_Class; class++)
		for(attribute = 0; attribute < datos -> N_IN; attribute++)
			nb -> variances[class][attribute] /= nb -> classProb[class];
  ...
  printf("Validacion:%f%%\nTest:%f%%\n",valid_error*100.,test_error*100.);
	...
	if(CONTROL){
		...
		for(attribute = 0; attribute < datos -> N_IN; attribute++){
			...
			for(class = 0; class < nb -> N_Class; class++)
				printf("\tClase %d: %f", class, nb -> means[class][attribute]);
			printf("\n");
		}
		...
		for(attribute = 0; attribute < datos -> N_IN; attribute++){
			printf("Atributo %d:", attribute);
			for(class = 0; class < nb -> N_Class; class++)
				printf("\tClase %d: %f", class, nb -> variances[class][attribute]);
			printf("\n");
		}
